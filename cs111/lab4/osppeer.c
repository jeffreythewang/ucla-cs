...
typedef struct task {
	...
	int bufSize;    // Holds size of the task's buffer
	unsigned head;
	unsigned tail;
...
static task_t *task_new(tasktype_t type)
{
	...
	t->total_written = 0;
	t->peer_list = NULL;
	t->bufSize = TASKBUFSIZ;
	t->buf = (char *)malloc(sizeof(char)*t->bufSize);
...
// task_buf_grow(t)
static void* task_buf_grow(task_t *t)
{
  t->bufSize *= 2;
  t->buf = realloc(t->buf, sizeof(char)*t->bufSize);
  return t->buf;
}
...
static char* get_md5(char *filename)
{
	int fd = open(filename, O_RDONLY);
	struct stat fileStats;
	if(fstat(fd, &fileStats))
	{
		error("* Error in file status.");
		exit(1);
	}
	size_t size = fileStats.st_size;
	char *data = malloc(sizeof(char)*size);
  int amt = read(fd, data, size);
	if (amt == -1 && (errno == EINTR || errno == EAGAIN
				|| errno == EWOULDBLOCK))
	{
		error("* Error in reading file contents for checksum.");
		exit(1);
	}
	close(fd);
	md5_state_t *pms = malloc(sizeof(md5_state_t));
	md5_init(pms);
	md5_append(pms, (const md5_byte_t *) data, size);
	char *result = malloc(sizeof(char)*(MD5_TEXT_DIGEST_SIZE + 1));
...
{
	unsigned headpos = (t->head % t->bufSize);
	unsigned tailpos = (t->tail % t->bufSize);
	ssize_t amt;

	if (t->head == t->tail || headpos < tailpos)
		amt = read(fd, &t->buf[tailpos], t->bufSize - tailpos);
	else
...
taskbufresult_t write_from_taskbuf(int fd, task_t *t)
{
	...
	if (t->head == t->tail)
		...
		amt = write(fd, &t->buf[headpos], t->bufSize - headpos);
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	while (1) {
		...
		if (ret == TBUF_ERROR) {
			...
			/* End of file */
			if (evil_mode) // Denial of service attack
				goto try_again;
			break;
		}
	...
	if (t->total_written > 0) {
		...
		char *dlchksum = get_md5(t->disk_filename);
		char *schksum = malloc(sizeof(char)*(MD5_TEXT_DIGEST_SIZE + 1));
		osp2p_writef(tracker_task->peer_fd, "MD5SUM %s\n", t->filename);
		read_tracker_response(tracker_task);		
		osp2p_snscanf(tracker_task->buf, MD5_TEXT_DIGEST_SIZE, "%s", schksum);
		if (strncmp(dlchksum, schksum, MD5_TEXT_DIGEST_SIZE) != 0)
		{
			message("* WARNING: The checksum of the downloaded file (%.*s) does not match the tracker checksum (%.*s). The file may be corrupted.\n", MD5_TEXT_DIGEST_SIZE, dlchksum, MD5_TEXT_DIGEST_SIZE, schksum);
			free(dlchksum);
			free(schksum);
			task_free(t);
			return;
...
static void task_upload(task_t *t)
{
	...
	if (memchr(t->filename, '/', FILENAMESIZ) != 0) {
		error("* Cannot serve files outside the current directory");
		goto exit;
	}
