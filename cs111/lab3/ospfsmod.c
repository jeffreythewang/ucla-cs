...
{
	/* EXERCISE: Your code here */
	if (blockno <= ospfs_super->os_firstinob + ospfs_super->os_ninodes * OSPFS_INODESIZE / OSPFS_BLKSIZE)
		return; //Named block out of bounds, return immediately
	void *vector = ospfs_block(OSPFS_FREEMAP_BLK);
...
add_block(ospfs_inode_t *oi)
{
	...
	if (n >= OSPFS_MAXFILEBLKS)
		return -ENOSPC;
	uint32_t firstAlloc;
	uint32_t secondAlloc;
	int32_t indir2Ind = indir2_index(n);
	int32_t indirInd = indir_index(n);
	int32_t dirInd = direct_index(n);
	if (indir2Ind == 0)
	{ //We are in the doubly indirect block. Alloc at least 1.
		uint32_t thirdAlloc;
		firstAlloc = allocate_block();
		if (firstAlloc == 0)
			return -ENOSPC;
		allocated[0] = firstAlloc;
		uint32_t *allocedBlkPtr = ospfs_block(thirdAlloc);
		memset(allocedBlkPtr, 0, OSPFS_BLKSIZE);
		if (dirInd == 0)
		{ //First entry of indirect block. Alloc at least 2.
			secondAlloc = allocate_block();
			if (secondAlloc == 0)
			{
				free_block(allocated[0]);
				return -ENOSPC;
			}
			allocedBlkPtr = ospfs_block(secondAlloc);
			memset(allocedBlkPtr, 0, OSPFS_BLKSIZE);
			allocated[1] = secondAlloc;
		}
		if (indirInd == 0)
		{ //First indir block of doubly indirect block. Alloc 3.
			thirdAlloc = allocate_block();
			if (thirdAlloc == 0)
			{
				free_block(allocated[0]);
			...
			allocedBlkPtr = ospfs_block(thirdAlloc);
			memset(allocedBlkPtr, 0, OSPFS_BLKSIZE);
		}
		if (indirInd == 0 && dirInd == 0) // Alloc doubly indir block.
			oi->oi_indirect2 = thirdAlloc;
		if (dirInd == 0) // Alloc indir block.
		{
			uint32_t *indirect = ospfs_block(oi->oi_indirect2);
			indirect[indirInd] = secondAlloc;
		}
		// Alloc direct block.
		uint32_t *indirect = ospfs_block(oi->oi_indirect2);
		uint32_t *direct = ospfs_block(indirect[indirInd]);
		direct[dirInd] = firstAlloc;
	}
	else if (indir2Ind == -1)
	{
		if (indirInd == 0)
		{ //We are in the singly indirect block. Alloc at least 1.
			firstAlloc = allocate_block();
			if (firstAlloc == 0)
				return -ENOSPC;
			uint32_t *allocedBlkPtr = ospfs_block(firstAlloc);
			memset(allocedBlkPtr, 0, OSPFS_BLKSIZE);
			allocated[0] = firstAlloc;
			if (dirInd == 0)
			{ //First entry of indirect block. Alloc 2.
				secondAlloc = allocate_block();
				if (secondAlloc == 0)
				{
					free_block(allocated[0]);
				...
				memset(allocedBlkPtr, 0, OSPFS_BLKSIZE);
				// Alloc indir block.
				oi->oi_indirect = secondAlloc;
			}
			// Alloc direct block.
			uint32_t *direct = ospfs_block(oi->oi_indirect);
			direct[dirInd] = firstAlloc;
		}
		else if (indirInd == -1)
		{ //We are in the direct section. Alloc exactly 1.
			firstAlloc = allocate_block();
			if (firstAlloc == 0)
				return -ENOSPC;
			uint32_t *allocedBlkPtr = ospfs_block(firstAlloc);
			memset(allocedBlkPtr, 0, OSPFS_BLKSIZE);
			oi->oi_direct[dirInd] = firstAlloc;
		}
	}
...
remove_block(ospfs_inode_t *oi)
{
	...
	int32_t dirInd = direct_index(n);
	if (indir2Ind == 0)
	{ //We are in the doubly indirect block. Dealloc at least 1.
		uint32_t indirect2 = oi->oi_indirect2;
		uint32_t *indirectPtr = ospfs_block(oi->oi_indirect2);
		uint32_t indirect = indirectPtr[indirInd];
		uint32_t *directPtr = ospfs_block(indirect);
		uint32_t direct = directPtr[dirInd];
		free_block(direct);
		directPtr += dirInd;
		directPtr = 0;
		if (dirInd == 0)
		{ //First entry of indirect block. Dealloc at least 2.
			free_block(indirect);
			indirectPtr += indirInd;
			indirectPtr = 0;
		}
		if (indirInd == 0)
		{ //First indir block of doubly indirect block. Dealloc 3.
	...
	{
		if (indirInd == 0)
		{ //We are in the singly indirect block. Dealloc at least 1.
			uint32_t indirect = oi->oi_indirect;
			uint32_t *directPtr = ospfs_block(oi->oi_indirect);
			uint32_t direct = ospfs_block(directPtr[dirInd]);
			free_block(direct);
			directPtr += dirInd;
			directPtr = 0;
			if (dirInd == 0);
			{ //First entry of indirect block. Dealloc 2.
				free_block(indirect);
				oi->oi_indirect = 0;
			}
		}
		else if (indirInd == -1)
		{ //We are in the direct section. Dealloc exactly 1.
			free_block(oi->oi_direct[dirInd]);
			uint32_t *dir = oi->oi_direct + dirInd;
			dir = 0;
		}
	}
	oi->oi_size -= OSPFS_BLKSIZE;
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
        ...
        if (bytes_left > OSPFS_BLKSIZE)
            copy_size = OSPFS_BLKSIZE - block_offset;
        else
            copy_size = bytes_left;

        uint32_t bytes_not_transferred = copy_to_user(buffer, data, copy_size);
        if (bytes_not_transferred != 0)
            retval = -EFAULT;

        n = copy_size - bytes_not_transferred;
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		uint32_t bytes_not_transferred = copy_from_user(data, buffer, copy_size);
		if (bytes_not_transferred != 0)
			retval = -EFAULT;
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	for (i = 0; i < dir_oi->oi_size; i += OSPFS_DIRENTRY_SIZE) {
		ospfs_direntry_t *cur_direntry = ospfs_inode_data(dir_oi, i);
		if (cur_direntry->od_ino == 0)
			return cur_direntry;
	}

	uint32_t new_size = (ospfs_size2nblocks(dir_oi->oi_size) + 1) * OSPFS_BLKSIZE;
	int change_status = change_size(dir_oi, new_size);
	if (change_status == 0) {
		dir_oi->oi_size = new_size;
		return ospfs_inode_data(dir_oi, i + OSPFS_DIRENTRY_SIZE);
	}
	else
		return ERR_PTR(change_status);
}
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	for (j = 2; j < ospfs_super->os_ninodes; j++) {
		ospfs_inode_t *cur_inode = ospfs_inode(j);
		if (cur_inode->oi_nlink == 0) {
			entry_ino = j;
			cur_inode->oi_size = 0;
			cur_inode->oi_ftype = OSPFS_FTYPE_REG;
			cur_inode->oi_nlink = 1;
			cur_inode->oi_mode = mode;
			cur_inode->oi_direct[0] = 0;
			ospfs_direntry_t *new_direntry = create_blank_direntry(dir_oi);
			if (IS_ERR(new_direntry))
				return PTR_ERR(new_direntry);
			new_direntry->od_ino = j;
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	for (j = 2; j < ospfs_super->os_ninodes; j++) {
		...
		if (cur_inode->oi_nlink == 0) {
			entry_ino = j;
			cur_inode->oi_size = dentry->d_name.len;
			cur_inode->oi_ftype = OSPFS_FTYPE_SYMLINK;
			cur_inode->oi_nlink = 1;
			ospfs_direntry_t *new_direntry = create_blank_direntry(dir_oi);
			if(IS_ERR(new_direntry))
				return PTR_ERR(new_direntry);
	...
	uint32_t symlen = strlen(symname);
	new_symlink->oi_size = symlen;
	new_symlink->oi_ftype = OSPFS_FTYPE_SYMLINK;
	new_symlink->oi_nlink = 1;
	strncpy(new_symlink->oi_symlink, symname, symlen);
...
MODULE_AUTHOR("Matthew Goldberg and Jeffrey Wang");
